          ...
          if result.id_ then
            ...
            if not database:get("lang:gp:" .. msg.chat_id_) then
              ...
              elseif is_sudoid(result.id_) then
                t = "مطوري حمبي 🦁"
              elseif is_admin(result.id_) then
                t = "ادمن في البوت حمبي🐒"
              elseif is_owner(result.id_, msg.chat_id_) then
                t = "مدير المجموعه حمبي🐰"
              elseif is_momod(result.id_, msg.chat_id_) then
                t = "ادمن جمبي🐱"
              elseif is_vipmem(result.id_, msg.chat_id_) then
                t = "عضو مميز حمبي🐔"
        ...
        if text:match("^[Pp]romote (.*)$") and check_user_channel(msg) then
          local promote_by_id = function(extra, result)
            if result.id_ then
              ...
              if database:sismember(hash, result.id_) then
                if database:get("lang:gp:" .. msg.chat_id_) then
                  ...
                  sendmen(msg.chat_id_, msg.id_, "🔖↓ العضو  " .. te .. "\n\n🔖↓ تم ✅ رفعه ادمن حمبي 🎈", 8, string.len(tp), result.id_)
                end
              else
                if database:get("lang:gp:" .. msg.chat_id_) then
            ...
            elseif database:get("lang:gp:" .. msg.chat_id_) then
              ...
              send(msg.chat_id_, msg.id_, 1, "➢  لا يوجد مستخدم حمبي 📍 ",  1, "md")
            end
          end
            ...
            if result.id_ then
              ...
              if not database:sismember(hash, result.id_) then
                if database:get("lang:gp:" .. msg.chat_id_) then
                  ...
                  sendmen(msg.chat_id_, msg.id_, "🔖↓  العضو  " .. te .. "\n\n🔖↓ تم تنزيله من الادمنيه حمبي🎈", 8, string.len(tp), result.id_)
                end
              else
            ...
            if result.id_ then
              ...
              if database:sismember(hash, result.id_) then
                if database:get("lang:gp:" .. msg.chat_id_) then
                  ...
                  sendmen(msg.chat_id_, msg.id_, "🔖↓ العضو  " .. te .. "\n\n🔖↓ تم رفعه عضو مميز حمبي❗️",  8, string.len(tp), result.id_)
                end
              else
                if database:get("lang:gp:" .. msg.chat_id_) then
              ...
              if result.id_ then
                ...
                if not database:sismember(hash, result.id_) then
                  if database:get("lang:gp:" .. msg.chat_id_) then
                    ...
                    sendmen(msg.chat_id_, msg.id_, "🔖↓  العضو  " .. te .. "\n\n🔖↓ تم تنزيله من المميزين حمبي🎈", 8, string.len(tp), result.id_)
                  end
                else
                  database:srem(hash, result.id_)
                  if database:get("lang:gp:" .. msg.chat_id_) then
...
if text ==   'بوسه'   then
if not database:get('bot:rep:mute'..msg.chat_id_) then
				keeper = "مِٰـﮧِۢﯛ̲୭ خٰ̐ـﮧِۢآفٰ͒ـِﮧۢ عِٰـﮧِۢيِٰـﮧِۢبِٰـِﮧۢ ₎❁😻🌸⇣"
else
                keeper = ''
end
send(msg.chat_id_, msg.id_, 1, keeper, 1, 'html')
...
if text ==   'test'   then
if not database:get('bot:rep:mute'..msg.chat_id_) then
				keeper = "<b>  حہٻًيہٻًيہ تہٳجہ رٳسہيہ ٳنٍيہ ٻًﯝتہ مْٳتہريہكُہسہ ٳٻًدٍ مْٳ ٳﯝكُہفُہ ﴿┊🙎🏼💛Ֆء﴾ۗ </b>"
else
                keeper = ''
end
send(msg.chat_id_, msg.id_, 1, keeper, 1, 'html')
...
if text ==   'ماتركس'   then
if not database:get('bot:rep:mute'..msg.chat_id_) then
				keeper = "اٰيٰہٰٖ مٰہٰٖاٰتٰہٰٖرٰيٰہٰٖكٰہٰٖسٰہٰٖ تٰہٰٖفٰہٰٖضٰہٰٖلہٰٖ حٰہٰٖمٰہٰٖبٰہٰٖيٰہٰٖ ℡ 🙈💚‏‎‏ ⁾┊❥"
else
                keeper = ''
end
send(msg.chat_id_, msg.id_, 1, keeper, 1, 'html')
...
✺.<b> الرابــط</b> : https://telegram.org/deactivate
   ...
   local text =  [[
<b>(اهــلاً بـك فــي سورس ماتريكس)</b> 🍃

<b>(السورس من تطوير🎣 )</b>:


<code>▪️مستر </code> : @sl00ls
<code>▫️تواصــل </code> : @sl00lsbot
...
<b>ﭑهلإ بــك في سورس matrix🏌️</b>
ﮧ━─────────━ﮧ
